...
let selectedVisitLogIds = new Set();
...
function bindEvents() {
  ...
  visitHistoryDeleteSelectedButton.addEventListener("click", deleteSelectedVisitLogs);
...
function openVisitLogTab(appointmentId = "") {
  switchScreen("appointments");
  setSubtab("appointments", "form");
...
function handleProviderSubmit(event) {
  ...
  const specialty = cleanText(formData.get("specialty")) || "General";
...
function handleVisitHistoryEditorClick(event) {
  const selectedCheckbox = event.target.closest("[data-select-visit-log]");
  ...
  if (selectedCheckbox) {
    if (selectedCheckbox.checked) {
      selectedVisitLogIds.add(selectedCheckbox.dataset.selectVisitLog);
    } else {
      selectedVisitLogIds.delete(selectedCheckbox.dataset.selectVisitLog);
    }
  ...
  if (deleteInlineEditId) {
    ...
    deleteVisitLogsByIds([deleteInlineEditId]);
    return;
  }
  ...
  if (deleteProviderId) {
    ...
    const idsToDelete = getAllVisitHistoryEntries()
      ...
      .map((entry) => entry.id);
    deleteVisitLogsByIds(idsToDelete);
    return;
  }
        ...
        ${
          visitHistorySelectMode
            ? `<label class="select-check">
                <input type="checkbox" data-select-visit-log="${entry.id}" ${selectedVisitLogIds.has(entry.id) ? "checked" : ""} />
                Select
              </label>`
            : ""
        }
...
function renderVisitHistoryGlobalList() {
  ...
  selectedVisitLogIds = new Set([...selectedVisitLogIds].filter((id) => entries.some((entry) => entry.id === id)));
  renderVisitHistoryControls();
...
function renderVisitHistoryControls() {
  const selectedCount = selectedVisitLogIds.size;
  visitHistoryFilterButtons.forEach((button) => {
...
function deleteSelectedVisitLogs() {
  const idsToDelete = [...selectedVisitLogIds];
  if (!idsToDelete.length) {
    return;
  }

  const confirmed = window.confirm(
    `Delete ${idsToDelete.length} selected visit log${idsToDelete.length === 1 ? "" : "s"}? This cannot be undone.`
  );
  if (!confirmed) {
    return;
  }
...
function deleteAllVisitLogs() {
  ...
  deleteVisitLogsByIds(entries.map((entry) => entry.id));
}

function deleteVisitLogsByIds(idsToDelete) {
  ...
  selectedVisitLogIds = new Set([...selectedVisitLogIds].filter((id) => !deleteIds.has(id)));
  visitHistorySelectMode = false;
  persist();
  render();
}
